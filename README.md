...
- **高精度RAG问答**：融合多路检索与重排算法（相对阈值动态裁剪），MQ增量同步知识库，搭建问答评估体系，优化问答准确率与忠实度。

- **长会话历史压缩**：对话超过阈值（默认 8 轮）后，先用轻量模型把最早的若干轮压缩成滚动摘要再删除原文，实测 token 节省约 92%；**摘要未生成出来时保留原文暂不删除、下轮重试**，绝不因压缩异常静默丢失对话。
